...
use algebra::secq256k1::Secq256k1BulletproofGens;
...
impl BulletproofURS for BulletproofParams {
    fn load() -> Result<BulletproofParams> {
        let urs = BULLETPROOF_CURVE25519_URS.ok_or(Error::MissingSRSError)?;

        let pp: BulletproofParams =
            bincode::deserialize(urs).map_err(|_| Error::DeserializationError)?;
        Ok(pp)
    }
...
impl BulletproofURS for Secq256k1BulletproofGens {
    fn load() -> Result<Self> {
        let urs = BULLETPROOF_SECQ256K1_URS.ok_or(Error::MissingSRSError)?;
...
impl BulletproofURS for ZorroBulletproofGens {
    fn load() -> Result<Self> {
        let urs = BULLETPROOF_ZORRO_URS.ok_or(Error::MissingSRSError)?;
