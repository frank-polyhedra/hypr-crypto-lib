...
use crate::parameters::{AddressFormat, SRS};
use algebra::bn254::BN254G1;
use algebra::secq256k1::{PedersenCommitmentSecq256k1, Secq256k1BulletproofGens};
use algebra::utils::save_to_file;
...
use crate::parameters::bulletproofs::{BulletproofParams, BulletproofURS};
...
use algebra::zorro::ZorroBulletproofGens;
use rayon::prelude::*;
use serde::Serialize;

#[derive(StructOpt, Debug)]
#[structopt(
    about = "Crypto tool to generate necessary zero-knowledge proof parameters.",
...
fn gen_transfer_vk(directory: PathBuf, address_format: AddressFormat) {
    println!(
        "Generating verifying keys for anonymous transfer for 1..{} payers, 0..{} payees ...",
    ...
    is.par_iter().for_each(|i| {
        ...
        let js: Vec<usize> = (0..=max_receiver).map(|j| j).collect();
