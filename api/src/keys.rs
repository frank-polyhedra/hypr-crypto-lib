...
impl FromToBytes for PublicKey {
    ...
    fn from_bytes(bytes: &[u8]) -> core::result::Result<PublicKey, AlgebraError> {
    ...
    pub fn verify(&self, message: &[u8], signature: &Signature) -> Result<()> {
        match (self.0, signature) {
            (PublicKeyInner::Ed25519(pk), Signature::Ed25519(sign)) => {
                pk.verify(message, sign).map_err(|_| Error::SignatureError)
            }
            (PublicKeyInner::Secp256k1(pk), Signature::Secp256k1(sign, _)) => {
                let mut hasher = Keccak256::new();
                hasher.update(message);
                let res = hasher.finalize();
                let msg = Message::parse_slice(&res[..]).map_err(|_| Error::SignatureError)?;
            ...
            (PublicKeyInner::EthAddress(hash), Signature::Secp256k1(sign, rec)) => {
                ...
                let res = hasher.finalize();
                let msg = Message::parse_slice(&res[..]).map_err(|_| Error::SignatureError)?;
                let pk = recover(&msg, sign, rec).map_err(|_| Error::SignatureError)?;
...
impl FromToBytes for SecretKey {
    fn to_bytes(&self) -> Vec<u8> {
    ...
    fn from_bytes(bytes: &[u8]) -> core::result::Result<SecretKey, AlgebraError> {
        // Compatible with old data.
        if bytes.len() == 32 {
...
impl SecretKey {
    ...
    pub fn sign(&self, message: &[u8]) -> Result<Signature> {
        match self {
            ...
            SecretKey::Secp256k1(sk) => {
                ...
                // it needs to be dealt with first, only hash in Crypto.
                let mut hasher = Keccak256::new();
                hasher.update(message);
                let res = hasher.finalize();
                let msg = Message::parse_slice(&res[..]).map_err(|_| Error::SignatureError)?;
    ...
    pub fn from_secp256k1_with_address(bytes: &[u8]) -> Result<Self> {
        let sk = Secp256k1SecretKey::parse_slice(bytes).map_err(|_| Error::DeserializationError)?;
...
impl FromToBytes for KeyPair {
    fn to_bytes(&self) -> Vec<u8> {
        let mut vec = vec![];
        vec.extend_from_slice(self.sec_key.to_bytes().as_slice());
        vec.extend_from_slice(self.pub_key.to_bytes().as_slice());
        vec
    }

    fn from_bytes(bytes: &[u8]) -> core::result::Result<Self, AlgebraError> {
        if bytes.len() == 64 {
            Ok(KeyPair {
            ...
            Ok(KeyPair {
                sec_key: SecretKey::from_bytes(&bytes[0..SECRET_KEY_LENGTH])?,
                pub_key: PublicKey::from_bytes(&bytes[SECRET_KEY_LENGTH..])?,
            })
        }
    }
}
...
impl KeyPair {
    ...
    pub fn generate_secp256k1_from_bytes(bytes: &[u8]) -> Result<Self> {
        let sk = Secp256k1SecretKey::parse_slice(bytes).map_err(|_| Error::ParameterError)?;
        let pk = Secp256k1PublicKey::from_secret_key(&sk);
        Ok(KeyPair {
            pub_key: PublicKey(PublicKeyInner::Secp256k1(pk)),
            sec_key: SecretKey::Secp256k1(sk),
...
impl FromToBytes for Signature {
    fn to_bytes(&self) -> Vec<u8> {
...
impl SignatureList {
    ...
    pub fn sign(keypairs: &[&KeyPair], message: &[u8]) -> Result<Self> {
        ...
        sorted.sort_unstable_by_key(|kp| kp.pub_key.to_bytes());
    ...
    pub fn verify(&self, pubkeys: &[&PublicKey], message: &[u8]) -> Result<()> {
        ...
        sorted.sort_unstable_by_key(|k| k.to_bytes());
