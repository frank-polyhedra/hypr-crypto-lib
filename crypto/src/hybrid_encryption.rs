...
impl FromToBytes for XPublicKey {
...
impl FromToBytes for XSecretKey {
    fn to_bytes(&self) -> Vec<u8> {
...
impl FromToBytes for HybridCiphertext {
    fn to_bytes(&self) -> Vec<u8> {
        let mut bytes = vec![];
        bytes.append(&mut self.ephemeral_public_key.to_bytes());
        bytes.append(&mut self.ciphertext.to_bytes());
        bytes
    }
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < 32 {
            ...
            let ephemeral_public_key = XPublicKey::from_bytes(&bytes[0..32])?;
            let ciphertext = CompactByteArray::from_bytes(&bytes[32..])?;
            Ok(Self {
                ciphertext,
