    ...
    fn multi_exp(scalars: &[&Self::ScalarType], points: &[&Self]) -> Self {
        ...
        unsafe {
            if WASM_INSTANCE.is_none() {
                web_sys::console::log_1(&"FastMSM for WASM has not been initialized, and the proof generation will use the Rust-based implementation. To initialize FastMSM, call the async WASM function `init()` to initialize the Crypto library before proof generation.".into());
